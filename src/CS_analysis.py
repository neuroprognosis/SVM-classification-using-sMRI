...
# -*- coding: utf-8 -*-
"""
Created on Sun Jan 16 23:18:46 2022
...
"""
def readDataRes(path, regex_match, mask, smooth):
    subjects_id = []
    list_files  = []
...
path = '/storage/users/agupta/unmodulated_segments/2mm/'
mask = nib.load('/storage/users/agupta/mask/icvmask.nii')
mat = readMatData('/storage/users/agupta/delcode_cov1079.mat')


data, subjects_id = readDataRes(path, 'wp1_*', mask, 6)

# create a dataframe and store image data and map it to subject IDs
subjects = pd.DataFrame()
subjects['subjs'] = np.array(subjects_id)
subjects['diag'] = subjects.subjs.map(mat.set_index('subj_id')['diag'])
subjects = subjects.dropna() 
data1 = data[subjects.index]
label = np.array(subjects.diag, dtype = 'int64')

# data filtering for binary classification      
data1 = data1[subjects.diag.isin([0,2])]
label1 = label[subjects.diag.isin([0,2])]
...
X_train, X_test, y_train, y_test = train_test_split(data1, label_bin, test_size=0.33, stratify=label_bin)


# data filtering for multiclass classification
data1_f = data1[subjects.diag.isin([0,1,2])]
label_f = label[subjects.diag.isin([0,1,2])]
X_train, X_test, y_train, y_test = train_test_split(data1_f, label_f, test_size=0.33, stratify=label_f)

# binarize the class labels for plottiong ROCs
label_bin_m = label_binarize(label_f, classes=[0, 1, 2]) 
X_train, X_test, y_train, y_test = train_test_split(data1_f, label_bin_m, test_size=0.33, stratify=label_bin_m)
...
for item in comparison_items:
    ...
    cv_score = model_selection.cross_validate(grid_search, item["X"], item["y"], cv=skf, scoring=scoring, n_jobs=-1)
    ...
    classifier = grid_search.fit(item["X"], item["y"])
    #optimum parameters
    print(classifier.best_params_)
    
    #plot CV ROCs
    draw_cv_roc_curve(classifier, skf, item["X"], item["y"], 
                      title = 'ROC for {}'.format(item["name"]))
tac()                       
...
for item in comparison_items:
    ...
    if(item["name"]=='weighted_SVM'):
      print('Weighted SVM Classification')
      clf = SVC(kernel='linear', probability=True, class_weight = 'balanced')
    else:
      print('Standard SVM Classification')
      clf = SVC(kernel='linear',C=50, probability=True) 
...
colorset=cycle(['mediumblue', 'crimson', 'purple', 'orange'])
# plot the results
...
for item in comparison_items: 
    ...
    if(item["name"]=='weighted_SVM'):
        ...
        print('Standard SVM')
        clf = OneVsRestClassifier(SVC(kernel='linear', probability=True))    
    draw_multi_roc_curve(classifier, item["X"], item["y"], title = 'ROC for {}'.format(item["name"]))
...
for item in comparison_items:
    ...
    clf = svm.fit(item['X'], item['y'])
    step_factor = 0.05 
    threshold_value = 0.1 
    roc_score=0
    predicted_proba = svm.predict_proba(X_test) #probability of prediction
    ...
    print('---Optimum Threshold for', item["name"],thrsh_score,'--ROC--',roc_score)
